...
double complex spheHarm(const int l, const int m, const double x, const double phi, int * const rstatus)
{
  gsl_sf_result result;
  double complex Ylm;
  int status = gsl_sf_legendre_sphPlm_e(l, fabs(m), x, &result);
  if(status == 0) {
